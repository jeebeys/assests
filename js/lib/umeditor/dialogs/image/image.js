(function () {
	...
	function initUmeditorQiniuWebUploader(me) {
		...
		var qiniuFlashUrl=me.editor.getOpt('qiniuFlashUrl');
		...
		$.post(qiniuTokenUrl,function(data){
			UM.UmeUploader  = WebUploader.create({
				auto : true,
				swf : qiniuFlashUrl||'http://files-res.jeebey.com/smartadmin/js/plugins/webuploader/Uploader.swf',
				server : qiniuUploadUrl,
				formData : {"token" :data[qiniuTokenKey||'uptoken'] ,'accept' : 'text/plain'},
				accept : {title : 'Images',extensions : 'gif,jpg,jpeg,bmp,png,jpg',mimeTypes : 'image/gif,image/jpg,image/jpeg,image/bmp,image/png,image/jpg'}
			});
			
			UM.UmeUploader.on('uploadBeforeSend', function( block, data, headers) {
    		    data.key = (qiniuImgPrefix||'')+Math.uuid(19);
    		});
